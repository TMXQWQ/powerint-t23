...
static NTP_Handler(struct Socket *socket, void *base) {
  // printf("Recv: ");
...
void UnNTPTimeStamp(uint32_t timestamp, uint32_t *year, uint32_t *month,
  ...
  *year = y + 1;
  uint32_t month0;
  if ((*year % 4 == 0 && *year % 100 != 0) || *year % 400 == 0) {
    for (int i = 0; timestamp > table2[i] * DAY_SEC; i++) {
      timestamp -= table2[i] * DAY_SEC;
      month0 = i + 2;
    }
  } else {
    for (int i = 0; timestamp > table1[i] * DAY_SEC; i++) {
      timestamp -= table1[i] * DAY_SEC;
      month0 = i + 2;
    }
  }
  *month = month0;
