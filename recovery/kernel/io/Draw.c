...
void putfont_cons(vram_t* vram,
                  int xsize,
                  int x,
                  int y,
                  color_t c,
                  char* font) {
  boxfill(vram, xsize, 0, x, y, x + 8, y + 16);
  int i;
  vram_t *p, d /* data */;
...
void putfonts_asc_cons(vram_t* vram,
  ...
  extern int cons_x, cons_y;
  char* hankaku = ascfont;
  /* C语言中，字符串都是以0x00结尾 */
  io_cli();
  for (; *s != 0x00; s++) {
    // printk("(y-24)/16=%d (x-5)/8=%d\n", (y - 24) / 16, (x - 5) / 8);
    if (*s == 0x0a || (*s == 0x0d && *(s + 1) == 0x0a) || (x - 5) / 8 == 80) {
      // printk("Endl\n");
      if (y == 24 + 16 * 24) {
        screen_ne_cons();
      } else {
    ...
    if (*s == 0x0d) {
      ...
      boxfill(vram, xsize, 0, x, y, x + 8, y + 16);
      sheet_refresh(sht_win_, x, y, x + 8, y + 16);
      sheet_refresh(sht_win, x, y, x + 8, y + 16);
    } else {
      if (*s != '\b') {
        putfont_cons(vram, xsize, x, y, c, hankaku + *s * 16);
        sheet_refresh(sht_win_, x, y, x + 8, y + 16);
        sheet_refresh(sht_win, x, y, x + 8, y + 16);
        x += 8;
      } else {
        // 画个8*16的黑色矩形
        boxfill(vram, xsize, 0, x - 8, y, x + 8, y + 16);
        sheet_refresh(sht_win_, x - 8, y, x + 8, y + 16);
        sheet_refresh(sht_win, x - 8, y, x + 8, y + 16);
      }
    }
  }
  cons_x = (x - 5) / 8;
