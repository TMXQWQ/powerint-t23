...
void High_Screen_ne(void) {
  ...
  memset(scr_buf + vinfo->xsize * (vinfo->ysize - 16) * sizeof(color_t), 0,
         vinfo->xsize * 16 * sizeof(color_t));
  sheet_refresh(sht_scr, 0, 0, vinfo->xsize, vinfo->ysize);
  bx = 0;
  by = vinfo->ysize / 16 - 1;
}
  ...
  for (i = 0; i < length; i++) {
    ...
    if (str[0] == 0x0d && str[1] == 0x0a) {
      ...
      if (bx > 0) {
        ...
        int bmx = bx;
        int bmy = by;
        SDraw_Box(scr_buf, bx * 8, by * 16, bx * 8 + 8, by * 16 + 16,
                  COL_000000, vinfo->xsize);
        bx++;
        cur_x += 8;
    ...
    if (GetCHorEN(str) == 1) {
      ...
      PUTCHINESE0(scr_buf, bx * 8, by * 16, COL_FFFFFF, *(short *)str,
                  vinfo->xsize);
    ...
    int bmx = bx;
    int bmy = by;
    SDraw_Char(scr_buf, bx * 8, by * 16, c, COL_FFFFFF, vinfo->xsize);
    bx++;
    Raw_x++;
    put_sht(sht_scr, bmx * 8, bmy * 16, 1);
    cur_x += 8;
