...
void mt_taskswitch1() {
  ...
  for (int i = mt_tr1 / 8 - 103; i < tasknum + 1; i++) {
    ...
    if (task->level == 1 &&
      ...
      taskctl = mt_tr1;
      mt_tr1 += 8;
      // printf("Level:1 Name:%s Task:%d*8
...
void mt_taskswitch2() {
  ...
  for (int i = mt_tr2 / 8 - 103; i < tasknum + 1; i++) {
    ...
    if (task->level == 2 &&
      ...
      taskctl = mt_tr2;
      mt_tr2 += 8;
      // printf("Level:2 Name:%s Task:%d*8
      // EIP:%08x\n",task->name,taskctl/8,task->tss.eip);
      timer_settime(mt_timer2, 3);
      timer_settime(mt_timer1, 1);
      farjmp(0, taskctl);  // 跳转
...
void mt_taskswitch3() {
  ...
  for (int i = mt_tr3 / 8 - 103; i < tasknum + 1; i++) {
    ...
    if (task->level == 3 &&
      ...
      taskctl = mt_tr3;
      mt_tr3 += 8;
      // printf("Level:3 Name:%s Task:%d*8
      // EIP:%08x\n",task->name,taskctl/8,task->tss.eip);
      timer_settime(mt_timer3, 8);
      if (mt2flag == 1) {
        timer_settime(mt_timer2, 3);
...
struct TASK* _fork(int b) {
  ...
  TaskSetFIFO(task, kfifo, mfifo);
  change_page_task_id(task->sel / 8 - 103, stack - 32 * 1024, 32 * 1024);
  change_page_task_id(task->sel / 8 - 103, memman, 4 * 1024);
  change_page_task_id(task->sel / 8 - 103, kfifo, sizeof(struct FIFO8));
  change_page_task_id(task->sel / 8 - 103, mfifo, sizeof(struct FIFO8));
  change_page_task_id(task->sel / 8 - 103, kbuf, 4096);
  change_page_task_id(task->sel / 8 - 103, mbuf, 4096);
