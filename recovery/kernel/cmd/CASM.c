...
char *_Asm(const char *asm_code1, int addr) {
  ...
  } else if (strncmp(asm_code, "cmp ", 4) == 0 && asm_code[4] == '%') //寄存器
  {
    ...
    if (strncmp(asm_code + 5, "near", 4) == 0 &&
        asm_code[10] == '$') { // 近调用
      //获取立即数
      char temp[30] = {0};
      int i = 0;
      ...
      sprintf(Result, "e8 %02x %02x %02x %02x",
              (unsigned char)((num - addr - 5) & 0xff),
              (unsigned char)((num - addr - 5) >> 8),
              (unsigned char)((num - addr - 5) >> 16),
              (unsigned char)((num - addr - 5) >> 24));
    }
    ...
    if (asm_code[4] == '%') //寄存器
    {
    ...
    } else if (strncmp(asm_code + 4, "short", 5) == 0 &&
               asm_code[10] == '$') { // 短跳转
      //获取立即数
      char temp[30] = {0};
      int i = 0;
    ...
    } else if (strncmp(asm_code + 4, "near", 4) == 0 &&
               asm_code[9] == '$') { // 近跳转
      //获取立即数
      char temp[30] = {0};
      int i = 0;
      ...
      temp[i - 10] = '\0';
      int num = strtol(temp, NULL, 16);
      sprintf(Result, "e9 %02x %02x %02x %02x",
              (unsigned char)((num - addr - 5) & 0xff),
              (unsigned char)((num - addr - 5) >> 8),
              (unsigned char)((num - addr - 5) >> 16),
              (unsigned char)((num - addr - 5) >> 24));
