...
int run_bat(char *cmdline) {
  ...
  if (finfo != 0) {
    if (strcmp(".BAT", &name[strlen(name) - 4]) != 0) {
...
struct TASK *start_drv(char *cmdline) {
  ...
  if (finfo != 0) {
    if (strcmp(".DRV", &name[strlen(name) - 4]) != 0) {
    ...
    // Hari=C, C程序 代码拥有两个段的程序（代码段和数据段）
    {
      ...
      */
      ...
      struct TASK *this_task =
          AddTask(name,1, ((3 + forever_num * 2 + app_num * 2) * 8), 0x1b,
                  ((4 + forever_num * 2 + app_num * 2) * 8),
...
int cmd_app(char *cmdline) {
  ...
  if (finfo != 0) {
    if (strcmp(".BIN", &name[strlen(name) - 4]) != 0) {
      page_free((void *)name, 300);
      return 0;
    }
    // 代码段的物理内存必须是连续的
    ...
    // Hari=C, C程序 代码拥有两个段的程序（代码段和数据段）
    {
      ...
      */
      ...
      datsiz = *((int *)(p + 0x0010));
      dathrb = *((int *)(p + 0x0014));
      q = (char *)page_malloc(segsiz + 512 * 1024 * 4); //分配数据段的内存
      set_segmdesc(gdt + 3 + forever_num * 2 + app_num * 2, finfo->size - 1,
                   (int)p, AR_CODE32_ER);
      set_segmdesc(gdt + 4 + forever_num * 2 + app_num * 2,
                   segsiz - 1 + 512 * 1024 * 4, (int)q, AR_DATA32_RW);
      for (i = 0; i < datsiz; i++) {
        q[esp + i] = p[dathrb + i]; //这里通过头数据拷贝数据段数据
      }
   //   ChangeLevel(NowTask(),3);
      ...
      page_free((int)stack, 64 * 1024);
      page_free((int)p, finfo->size);
      page_free((int)q, segsiz + 512 * 1024 * 4);
      print("\n");
    } else if (elf32Validate(fp->buf)) {
      // printk("----------ProGram Running Malloc Info-----------");
      p = (char *)page_malloc(finfo->size);
