...
int format(char drive) {
  ...
  if (!(drive - 'A')) {
    ...
    *(unsigned char *)(&read_in[BS_DrvNum]) = 0;
    ...
    if (!info->CylinesNum) {
      // printf("Couldn't find IDE HardDisk.\n");
      return 1;
    }
    for (int i = 0; i != 40; i += 2) {
      char c = info->ID[i + 1];
      info->ID[i + 1] = info->ID[i];
      info->ID[i] = c;
    }
    *(unsigned char *)(&read_in[BPB_SecPerClus]) =
        ((info->CylinesNum * info->HeadersNum * info->TrackSectors * 512 /
          4096) /
             512 +
         1);
    *(unsigned short *)(&read_in[BPB_RootEntCnt]) =
        ...
        (((info->CylinesNum * info->HeadersNum * info->TrackSectors * 512 /
         ...
         512) /
        32;
    if (info->CylinesNum * info->HeadersNum * info->TrackSectors > 65535) {
      *(unsigned short *)(&read_in[BPB_TotSec16]) = 0;
    } else {
      *(unsigned short *)(&read_in[BPB_TotSec16]) =
          info->CylinesNum * info->HeadersNum * info->TrackSectors;
    }
    *(unsigned int *)(&read_in[BPB_TotSec32]) =
        info->CylinesNum * info->HeadersNum * info->TrackSectors;
    *(unsigned char *)(&read_in[BS_DrvNum]) = 0x80;
    drivers_idehdd_write(0, 1, (unsigned short *)read_in);
    unsigned int *fat = (unsigned int *)page_malloc(9 * 512);
    fat[0] = 0x00fffff0;
    drivers_idehdd_write(1, 9, (unsigned short *)fat);
    drivers_idehdd_write(10, 9, (unsigned short *)fat);
    page_free((void *)fat, 9 * 512);
    void *null_sec = page_malloc(512);
    clean((char *)null_sec, 512);
    for (int i = 0; i < 14 *
                            (((info->CylinesNum * info->HeadersNum *
                               info->TrackSectors * 512 / 4096) /
                                  512 +
                              1) *
                             512) /
                            32 * 32 / 512;
         i++) {
      drivers_idehdd_write(19, 1, (unsigned short *)null_sec);
    }
    page_free(null_sec, 512);
